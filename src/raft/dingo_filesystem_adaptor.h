...
const std::string kSnapshotDataFile = ".sst";
...
struct IteratorContext {
  ...
  std::shared_ptr<RawEngine::Reader> reader;
...
struct SnapshotContext {
  SnapshotContext() : snapshot(Server::GetInstance().GetRawEngine()->GetSnapshot()) {
    for (int i = 0; i < Helper::GetColumnFamilyNames().size(); i++) {
      data_iterators.push_back(std::make_shared<IteratorContext>());
    }
  }
  ~SnapshotContext() {
    if (snapshot != nullptr) {
  ...
  std::vector<IteratorContextPtr> data_iterators;
...
using SnapshotContextPtr = std::shared_ptr<SnapshotContext>;
...
class DingoDataReaderAdaptor : public braft::FileAdaptor {
 ...
 protected:
  DingoDataReaderAdaptor(int64_t region_id, const std::string& path, DingoFileSystemAdaptor* rs,
 ...
 private:
  ...
  std::string path_;
  DingoFileSystemAdaptor* rs_ = nullptr;
  IteratorContextPtr context_ = nullptr;
  bool closed_ = true;
  size_t num_lines_ = 0;
...
class DingoFileSystemAdaptor : public braft::FileSystemAdaptor {
 ...
 private:
  ...
  SnapshotContextPtr GetSnapshot(const std::string& path);
  ...
  using SnapshotMap = std::map<std::string, ContextEnv>;
